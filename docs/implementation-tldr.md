...
- Flags: `--can-interface <name>` (default `can0`), `--simulate` to force the simulator, `--kiosk` for borderless fullscreen with the cursor hidden (in-car display; `Esc` quits, only wired up in this mode).
